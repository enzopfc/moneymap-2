      ...
      <table className="min-w-full text-sm">
        <thead>
          <tr className="text-left bg-gray-100 text-gray-700">
            {colunas.map((c) => <th key={c.acesso} className="px-3 py-2 font-medium">{c.titulo}</th>)}
            {(onEditar || onExcluir) && <th className="px-3 py-2 font-medium">Ações</th>}
          </tr>
        </thead>
        <tbody>
          {dados.length === 0 && <tr><td className="px-3 py-3 text-gray-500" colSpan={colunas.length + 1}>Nenhum registro encontrado.</td></tr>}
          {dados.map((linha) => (
            <tr key={linha.id} className="border-b last:border-0">
              {colunas.map((c) => <td key={c.acesso} className="px-3 py-2">{c.render ? c.render(linha) : linha[c.acesso]}</td>)}
              {(onEditar || onExcluir) && (
